...
**Comments** (line 22) — комментарии под постами.

⭐ **Главный приоритет — открытая сущность B24 у клиента**. Иерархия:

| Open entities у клиента | Действие при incoming-комменте |
|---|---|
| 1+ открытая **сделка** | Сессия в линии 22 → прикреплена к **контакту**. Коммент **дублируется в timeline всех открытых сделок**. Лид НЕ создаётся. |
| Сделок нет, есть открытый **лид** | Сессия → прикреплена к **этому лиду**. Новый лид НЕ создаётся. |
| Ни сделки, ни лида | Создаётся **новый лид** + сессия прикреплена к нему. |

Это правило применяется:
- К **любому каналу IG** (Direct и Comments) — приоритет по контакту/лиду,
  не по линии
- К **любому посту** — **новый пост = новая открытая линия**, но
  прикреплённая к **существующему** лиду/контакту (новый лид не создаётся)

### Outgoing IG Direct из существующего чата (`!`-префикс)

Когда менеджер в Comment-сессии под пост A пишет `!Здравствуйте`:

- Сообщение уходит клиенту **в Direct** через i2crm
- Создаётся **новая Direct-сессия** в линии 18
- Direct-сессия прикреплена к **тому же лиду/контакту** (не плодим)
- Direct-сессия сразу **в работе у этого менеджера** (без нераспределённых)
- Если менеджер не отвечает 10 мин — auto-release в нераспределённые
  (общее правило, см. `SLA_RULES.md`)

### Сделки автоматом не создаются

**Сделка никогда не создаётся системой** — только менеджер вручную
конвертирует из лида когда есть смысл.

### Примеры

- Клиент пишет коммент под постом A впервые, никаких открытых
  сущностей → создан лид #1 + сессия A
- Клиент пишет коммент под **тем же постом A** → идёт в **ту же сессию A**
  (chat.id совпадает)
- Клиент пишет коммент под **новым постом B**, лид #1 ещё открыт →
  создаётся **сессия B** + прикреплена к **существующему лиду #1**.
  Итого: 1 лид, 2 сессии (A и B).
- Менеджер конвертил лид #1 в сделку. Клиент пишет коммент под постом C →
  создаётся **сессия C** + прикреплена **к контакту** + коммент
  **скопирован в timeline сделки**. Лид НЕ создан.
- Лид/сделок открытых не осталось (сделка выиграна и закрыта). Клиент
  пишет коммент → создаётся новый лид #2.

> ⚠️ **2026-05-26 (ADR [`2026-05-26-ig-comments-attach-to-open-entity`](./decisions/2026-05-26-ig-comments-attach-to-open-entity.md))**:
> правило изменилось с прежнего «один пост = один лид» на новое «открытая
> сущность приоритет». Реализация в коде — task #66.
