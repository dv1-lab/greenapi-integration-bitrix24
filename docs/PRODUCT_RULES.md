...
**один (клиент × пост) = одна сессия = один лид** (см. `INSTAGRAM_FLOW.md`).

- Клиент пишет коммент под постом A → создаётся лид #1 (под пост A)
- Тот же клиент пишет ещё коммент под постом A → идёт **в существующий
  лид #1** (та же сессия, действует общее правило `CRM_FORWARD=Y`)
- Тот же клиент пишет коммент под постом B → создаётся лид #2 (под пост B)
- Все лиды разных постов одного клиента **привязаны к одному контакту**
  через `UF_CRM_IG_CHAT_ID`

Логика: каждый пост обычно про разный товар (под Garmin — про Garmin,
под Suunto — про Suunto), смешивать в один лид = каша.
