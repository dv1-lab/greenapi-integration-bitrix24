...
Это правило применяется **независимо от того, под каким постом** клиент
пишет (новый пост ≠ новый лид). И **независимо от канала** (Comment или
Direct) — это поведение Бэк по контакту, не по chat.id.

Примеры:

- Открыта сделка по клиенту → клиент комментирует пост A → коммент в эту
  сделку. Комментирует пост B — тоже в эту сделку. Лиды НЕ создаются.
- Открытого лида/сделки нет → клиент комментирует пост A → создаётся
  новый лид. Комментирует пост B → идёт **в тот же лид** (он ещё открыт).
- Лид закрыт (converted, junk) и сделок открытых нет → новый коммент =
  новый лид.

Логика: лишние лиды плодить не нужно — у клиента уже есть открытая
переписка с менеджером, всё к ней.

> ⚠️ **2026-05-26 (ADR `2026-05-26-ig-comments-attach-to-open-entity`)**:
> правило изменилось с прежнего «один пост = один лид» на новое «открытая
> сущность приоритет». Реализация в коде — в TODO.
