...
**Источник истины — НЕ Telegram-зеркала** (уточнено 25.05):
- Adapter пишет события **напрямую** в `customer-service /events/ingest` →
  ClickHouse `customer_events`. TG-зеркало здесь не участвует.
- B24 open-lines заполняется **напрямую** через `imconnector.send.messages`.
  TG-зеркало здесь тоже не участвует.
- TG-зеркала (`wa-tg-bridge ig_bridge.py` и аналоги) — **дополнительный UI**
  для операторов «всё в одном Telegram-окне». Если зеркало упало —
  Customer-360 + B24 продолжают работать; операторам просто неудобно
  (надо открывать B24 для ответа). Не критично.
...
## 7.6. Правило создания лидов/сделок при обращениях

Зафиксировано как продуктовое правило 26.05.2026 (раньше существовало
устно, разные сессии повторяли вопрос «почему создался лид?»).
...
**Why**: дубли лидов «Повторный лид» для одного клиента ломали
CRM-метрики и плодили шум для операторов. Один-лид-на-намерение.

**Настройка B24** для каждой open-line (через `imopenlines.config.update`):

```
PARAMS[CRM]              = Y
PARAMS[CRM_CREATE]       = lead        # первое обращение — лид
...
→ создаёт повторный лид. Использовать строго `"N"`.

**Применено к 7 линиям** 26.05.2026: `18` (IG Direct), `22` (IG Comments),
`148`/`174` (WA), `178`/`204` (TG), `182` (MAX).

**При создании новой линии** — обязательно применить эту настройку
сразу, иначе B24 заведёт настройки по умолчанию и правило сломается
для этого канала. Команда:
...
См. также: memory `b24_lead_creation_rules`.
