...
**РЕШЕНО (sha `eb86048`+`0e70e81`, вечер 2026-07-12):** чат теперь садится В
исходный лид без дубля. Механизм — **дедуп B24 по телефону**, как у WhatsApp:
`mirrorToBitrix` кладёт `user.phone` в payload и для TG/MAX (раньше только WA),
`/widget/send` прокидывает телефон из карточки. B24 контролем дублей находит
открытый лид с этим номером и цепляет сессию к нему. `clientName` из карточки →
имя клиента в заголовке диалога (иначе chatId). Подтверждено на WA/TG/MAX (лид
364451, обе линии сели в него, `ONCRMLEADADD` не было).

⚠️ **Условие:** дедуп цепляет к открытому лиду по номеру, только если такой
открытый лид РОВНО ОДИН. Несколько дублей с одним номером (напр. накопились от
многократных тестов) → B24 не выбирает → создаёт новый. В норме (один лид на
человека) — привязывает. Не проверять «дубля нет» по `backfillSendLead: no
matching lead` — B24 создаёт лид под сессию позже 15-сек окна; смотреть реальный
список лидов.

**Cross-account (sha `c2db46e`):** UF_CRM_{TG|MAX}_CHAT_ID привязан к аккаунту —
слать по нему с ДРУГОГО аккаунта нельзя (Telethon «Could not find the input
entity» → 500). Виджет берёт chatId по ЛИНИИ инстанса; UF — только при одном
аккаунте провайдера. Кейс: офисный Telegram (204) против основного (178). См.
[`REGRESSIONS.md` 2026-07-12 вечер](./REGRESSIONS.md).
