...
- **Фикс** (sha `855f15d`): код таймаут не лечит (ретрай внутри 15с-окна
  виджета смысла нет). Вместо глухого 502 — в текст ошибки CheckAccount добавлена
  подсказка ввести `@username` клиента (отправка идёт в обход резолва номера,
  `usernameOverride` → `<username>@c.us` уже поддерживается). Файл:
  `src/widget/widget.controller.ts` (catch-блок CheckAccount, ~стр. 335).
...
- **Что НЕ ломать**: подсказка применима И для MAX, И для Telegram — оба
  принимают `<username>@c.us`. Не сужать до одного провайдера. Поле `@username`
  во фронте виджета уже есть (на него ссылается и соседняя ошибка «номер не
  найден» для Telegram).
