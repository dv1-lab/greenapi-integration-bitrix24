...
- **Verify:** задеплоен `c2db46e`, adapter поднят; повторный тест офисного TG — за Дмитрием (успех зависит от TG-privacy клиента, но 500 из-за чужого id исключён).
- **Не повторять:** `UF_CRM_{TG|MAX}_CHAT_ID` НЕ хранит, каким аккаунтом добыт — при >1 аккаунте одного провайдера нельзя слать по нему без проверки линии.
