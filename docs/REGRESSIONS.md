...
**Часть A — привязка по телефону (sha `eb86048` + `0e70e81`)**
- **Симптом:** «написать первым» в TG/MAX создавал отдельный лид под чат, исходный лид получал только timeline-комментарий. (WhatsApp так не делал.)
- **Корень:** payload `imconnector.send.messages` для TG/MAX НЕ содержал `user.phone` (клался только для WA — `mirrorToBitrix` хардкодил `provider==="wa"`). Без телефона контроль дублей B24 не находил открытый лид клиента → заводил новый. WA работал именно потому, что слал phone.
- **Фикс:** `mirrorToBitrix` получил необязательный `phoneOverride`; `/widget/send` прокидывает телефон из карточки и для TG/MAX → B24 своим дедупом по номеру цепляет сессию к открытому лиду. + `0e70e81`: имя клиента из карточки (`clientName`) в заголовок диалога (иначе показывался chatId).
- **Verify (логи прод, лид 364451):** payload `user:{id:sc_514948704,name:"Дмитрий",phone:"+79951145590"}`; обе линии (MAX 182 + TG 178) сели в #364451; `backfillSendLead: no matching lead found` + ни одного `ONCRMLEADADD` = B24 нового лида НЕ создал. Подтверждено на WA/TG/MAX.
- **ВАЖНО — условие работы:** дедуп B24 цепляет к открытому лиду по номеру, только если такой открытый лид РОВНО ОДИН. Несколько (накопились дубли, напр. от многократного теста на один номер) → B24 не выбирает → создаёт новый. В нормальном состоянии (один лид на человека) — привязывает.
- **Не повторять:** НЕ считать `backfillSendLead: no matching lead` доказательством «дубля нет» — B24 создаёт лид под сессию ПОЗЖЕ (при автозаборе), вне 15-сек окна backfill. Проверять реальный список лидов + `ONCRMLEADADD`. Не гонять многократные тесты на один номер, не чистя лиды — накопление открытых дублей ломает дедуп.

**Часть B — офисный Telegram слал chatId чужого аккаунта (sha `c2db46e`)**
- **Симптом:** «написать первым» с «Telegram Office +7 924 077-85-66» → `Green API: Request failed with status code 500`.
- **Корень:** виджет брал `UF_CRM_TG_CHAT_ID` (id, добытый ОСНОВНЫМ TG-аккаунтом, линия 178) не глядя, с какого аккаунта шлём. Офисный аккаунт (линия 204) этот user_id «не знает» → Telethon `Could not find the input entity for PeerUser(6748117222)` → 500. chatId в TG/MAX привязан к аккаунту (access_hash), между аккаунтами не переносится.
- **Фикс:** фронт виджета берёт chatId приоритетно по ЛИНИИ выбранного инстанса (`MAX_CHATS_BY_LINE`); UF-поле — только если у провайдера один аккаунт и chatId не засвечен под другой линией (`safeChatIdForLine`/`providerCount`). Иначе override не ставим → бэкенд резолвит заново через CheckAccount для выбранного аккаунта, внятная ошибка вместо 500.
- **Verify (подтверждено 21:58):** офисный TG (линия 204) → фронт пропустил чужой chatId → бэкенд CheckAccount офисного аккаунта → `ImportContacts` резолвит номер + кэширует entity (access_hash) → отправка OK (`tg6748117222_530`), чат сел в исходный лид 364451 («Дмитрий - Telegram Office»).
- **Компаундный корень (важно!):** ранние попытки (до фикса) закэшировали чужой chatId в таблицу `MaxContact (idInstance, phone)` — бэкенд брал его оттуда, МИНУЯ CheckAccount → 500 сохранялся даже после фронт-фикса, пока не почистили. Разово удалены отравленные office-строки (`DELETE FROM MaxContact WHERE idInstance=4100624465 AND chatId IN (6748117222,161220770)`). Первопричина кэша: `/widget/send` пишет `chatIdOverride` в `MaxContact` без проверки, что chatId принадлежит инстансу.
- **Не повторять:** `UF_CRM_{TG|MAX}_CHAT_ID` НЕ хранит, каким аккаунтом добыт — при >1 аккаунте одного провайдера нельзя слать по нему без проверки линии. И: не кэшировать `chatIdOverride` в `MaxContact` вслепую (бэкенд-хардненинг в бэклоге) — иначе кросс-аккаунтный id отравляет кэш.

**Часть C — «зависшая» open-line сессия на удалённом лиде (WA, диагностика, не фикс)**
- **Симптом:** WhatsApp write-first из #364451 → сообщение уходит (wuzapi 200, номер имеет WA), но open-line не появляется в лиде.
- **Корень:** B24 переиспользовал СТАРУЮ открытую WA-сессию (534260, с прошлых тестов), привязанную к УЖЕ УДАЛЁННОМУ лиду. **Ключевой инвариант:** переоткрытая (уже существующая открытая) сессия к новому лиду сама НЕ переезжает — привязка по телефону срабатывает только при создании НОВОЙ сессии. TG/MAX сегодня создавали свежие (536xxx), потому и сели в лид.
- **One-off:** оператор завершил диалог в UI (штатного REST-close у нас нет — только join/answer/transfer) → следующая отправка создала свежую сессию 536192 → села в #364451.
- **Verify:** в #364451 подтверждены ВСЕ 4 линии: WhatsApp + Telegram + Telegram Office + MAX, без дублей.
- **Системно (задача #10):** sweep-cron закрывать открытые сессии с удалённым лидом + хук ONCRMLEADDELETE; предусловие — найти способ закрыть сессию из кода. Процесс: не hard-delete-ить лид с живым диалогом.

**Диагностика по ходу (в бэклог, не фиксы):** «эхо»-лид на собственный номер 79584983354 (chatId 328839495) из потока outgoing-from-device — нужен гард «не заводить лид на свой инстанс-номер». Заголовки уже существующих диалогов B24 задним числом не переименовывает (косметика).

---

## 2026-07-12 · Write-first из холодного лида плодил несвязанный лид-дубль — ЧАСТИЧНО → РЕШЕНО (см. запись выше, phone-dedup)

- **sha:** `b9e56a9` (widget привязка к placement entity)
- **Симптом:** «написать первым» в TG/MAX из карточки холодного лида → сообщение уходит, но open-line создаётся в новом пустом лиде, с исходным лидом не связан, номер/имя не подтянуты.
- **Корень:** `ensureOpenLeadForPhone` (`bitrix24.service.ts`) ищет только КОНТАКТЫ (по phone / UF_CRM_*_CHAT_ID), лиды игнорирует; холодный лид без контакта → `return {}` → imconnector плодит несвязанный лид. Отличие от WhatsApp: WA передаёт phone в open-line payload, B24 матчит нативно к лиду с этим номером; TG/MAX phone не передают (privacy) → матчить нечем. Фронт виджета знал placement entity (lead), но не слал в `/widget/send`.
- **Фикс:** (1) фронт виджета шлёт `entityType`+`entityId` из placement; (2) новый `writeChatIdToEntity` пишет резолвнутый chatId в `UF_CRM_{TG|MAX|IG}_CHAT_ID` исходной сущности; (3) `placementEntity` как приоритетный `openEntity` → `backfillSendLead` закрывает imconnector-дубль как «[Дубликат → lead N]» (UF_CRM_LEAD_ID), подтягивает phone/имя, timeline в исходный лид. Работает без контакта (contactId опционален).
- **Verify:** smoke Дмитрия (лиды 364414/364424) — дубль помечен «[Дубликат → lead 364414]», стадия Дубликат, phone+timeline в исходный лид.
- **Что не повторять / ОТКРЫТО:** цель Дмитрия УТОЧНЕНА после smoke — нужен чат open-line В исходном лиде БЕЗ создания дубля вообще (сейчас чат остаётся на дубль-лиде). Прямых REST-методов перепривязки chat открытой линии в B24 нет (проверялось, см. widget_max_47). Требует research: перенос активности IMOPENLINES_SESSION на исходный лид / как incoming привязывает / как старые коннекторы делали. Текущий фикс (связка дубля) оставлен — не вредит, лучше прежнего. Не удалять дубль вслепую.
...
- **Симптом**: на чате 8566 «отвечал старый беговой коннектор», а должен social_connector.
- **Причина**: после полной миграции `wa_tg_bridge` B24-коннектор остался ACTIVE на линии
  **204** (Telegram 8566) одновременно с social_connector → B24 мог роутить исходящее в
  старый коннектор. На линии 148 (WhatsApp 8566) коннектор уже был выключен — премиса
  «дело в 148» была устаревшей. Диагностика через новый `internal/connector-status`
  (матрица imconnector.status по 5 линиям) показала единственную проблемную линию — 204.
- **Фикс**: `internal/connector-set-active` → `imconnector.activate CONNECTOR=wa_tg_bridge
  LINE=204 ACTIVE=0`. Переснятие матрицы: CONFIGURED:false/STATUS:false, social_connector
  активен. Теперь все 5 линий (148/174/178/182/204) — только на social_connector.
- **Затронутые файлы**: `bitrix24.service.ts` (`diagnoseConnectors`, `setConnectorActive`),
  `webhooks.controller.ts` (2 internal-эндпоинта под X-Hint-Secret).
- **Стабильность**: мост НЕ реактивирует коннектор при старте (только алертит при
  отсутствии регистрации в imconnector.list) → деактивация переживает рестарты моста.
- **Пере-сломать рискованно**: не деактивировать `wa_tg_bridge` БЕЗ проверки что
  social_connector активен на той же линии (иначе исходящие пропадут). Проверять матрицей
  до и после.

---

## 2026-06-23 · Зеркало omni-исходящих (Фикс C) задвоило B24-собственные — РЕШЕНО v2

- **РЕШЕНО (v2, `ca140cf` adapter + omnisocial `416e6f3`/`9f0c835`)**: триггер зеркала —
  отдельное поле `omniMirror` (ставит ТОЛЬКО прямой фан фасада `_fan_adapter_outgoing`,
  который теперь постит НАПРЯМУЮ в adapter-вебхук `social.9wb.ru/webhooks/green-api`),
  а НЕ `omniSource`. Мостовое эхо несёт только `omniSource` → обработчик его игнорит →
  дубля нет. Прямой фан → TG-топик тоже доходит. Загадка «прямой фан в /wh/ не сработал»
  объяснилась: `172.23.0.1:8732/wh/` — это ВТОРОЙ экземпляр wa-tg-bridge на my-server
  (релей), а не adapter; он форвардит только то, что показывает, не наш синтетический
  outgoing. Поэтому постим прямо в adapter. Имя клиента в зеркале — из кэша `_chat_names`
  (PushName из входящих). E2E зелёный. Ниже — история инцидента v1 (для контекста).

- **sha**: попытка v1 `81dd86f`, откат `6dcf87b`
- **Симптом**: после выката зеркала исходящих из кабинета/TG-топика в B24 — ответы
  оператора, отправленные ИЗ САМОГО B24, начали появляться в B24 дважды (оригинал +
  белое «<phone> <text>» self-message). TG-топик-исходящие при этом в B24 не доходили.
- **Корень**: `handleOutgoingFromCabinet` триггерился на поле `webhook.omniSource`.
  Но `omniSource=cabinet` ставит фасадный `_fan_cabinet_outgoing` для ОТОБРАЖЕНИЯ в
  TG-топике (prefix «↗️ из кабинета») — и для B24-собственных исходящих тоже (они
  идут той же elif-веткой фасада). Мост (wa-tg-bridge) ФОРВАРДИТ полученные вебхуки
  на `FORWARD_WEBHOOK_URL=social.9wb.ru/webhooks/green-api` (тот же adapter) → эхо
  B24-исходящего возвращалось в adapter с omniSource → зеркалилось обратно в B24 = дубль.
- **Фикс (временный)**: откат обработчика (`6dcf87b`). Дубль ушёл (B24-ответ «9999» →
  клиенту 1 раз, без зеркала).
- **Что НЕ повторять / план**: триггер зеркала вести по ОТДЕЛЬНОМУ полю `omniMirror`
  (ставит ТОЛЬКО прямой facade→adapter фан `_fan_adapter_outgoing`), НЕ по `omniSource`
  (его несёт мостовое эхо для показа). Тогда форвард моста не триггерит зеркало.
  Перед след. попыткой — разобрать, почему прямой `_fan_adapter_outgoing`→`/wh/<secret>`
  (POST 200 OK в логах facade) НЕ зазеркалил TG-топик (возможно `/wh/` и `/webhooks/green-api`
  обрабатываются по-разному, либо дедуп по idMessage). Полный диагноз — task «Фикс C».

## 2026-06-23 · Входящее медиа 8566 (omnisocial-фасад) не доходило в B24

- **sha**: `3e24534`
- **Симптом**: после увода боевого WhatsApp 8566 на наш движок (omnisocial) входящие
  фото/голос не появлялись в линии 148 B24 (текст доходил). В карточке — битая ссылка.
- **Корень**: метод приёма в B24 само-скачивает медиа и переотдаёт через `social.9wb.ru`
  (B24-облако не тянет шардовые URL Green API) — но условие ловило **только**
  `*.api.green-api.com`. Наш фасад отдаёт медиа с `facade-os.9wb.ru`, который под фильтр
  не попадал → adapter отдавал B24 прямую `facade-os` ссылку, а её облако B24 скачать не
  может (IP-фильтр facade-os только под server-spb).
- **Фикс**: расширил условие в `bitrix24.service.ts` (метод отправки в B24, ~стр 3849):
  `... || /facade-os\.9wb\.ru\//i.test(url) ...` → adapter качает facade-os медиа сам.
- **Verify**: прод-лог 23.06 — `Медиа Green API проксировано для B24: https://social.9wb.ru/media/…jpeg`
  → `imconnector.send.messages` SUCCESS, B24 сохранил файл.
- **Что НЕ повторять**: для surface-инстансов (omnisocial) в адаптере `apiTokenInstance` =
  **единый** `SURFACE_TOKEN` кабинета (роут `omnisocial.9wb.ru/api/green` проверяет один токен,
  не per-инстанс), а НЕ Green API-токен и НЕ per-инстанс `webhook_secret`. Ставить новому
  surface-инстансу токен, скопированный с уже рабочего (напр. 4099467) + добавить id в
  `OMNI_SURFACE_INSTANCES`. ОСТАЛОСЬ (Фикс C): кабинетные/TG-топик исходящие в B24 не зеркалятся —
  эхо не доходит до adapter, нужен `handleOutgoingFromCabinet` + фан эха в adapter + `X-Omni-Origin`.
