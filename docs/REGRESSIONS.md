...
## 2026-09-16 · Статусы доставки Instagram не писались никогда — guard выходил молча

- **Симптом.** Никакого. Ровно в этом дело: страница «зависших исходящих»
  дашборда показывала, что по Instagram всё в порядке, а в коде рядом стоял
  комментарий «`delivery_status=sent` … достаточно чтобы не показывал как
  зависшее». Замер на боевой базе: событий `message_delivery_status` с
  `channel='IG'` — **0 за всю историю**, при 37 097 исходящих всего и 986 по
  Instagram за последние 30 дней.
- **Корень.** `_emitMessageDeliveryEvent` пишет событие только если знает, к
  кому его отнести: нужен `customerUuid` **либо** `b24ChatId`, из которого
  достаётся alias. Все три вызова для IG в `handleI2crmOutgoing` передавали
  `idMessage/status/source/channel/connector` и ничего больше — guard выходил,
  записав `warn` в лог, и возвращал успех вызывающему. Отказ по построению
  тихий: `void` перед вызовом, best-effort по замыслу, ни одной ошибки наружу.
- **Фикс.** `b24ChatId: \`i2crm_ig_${clientId}\`` во всех трёх вызовах, плюс
  `sent`/`failed` в новом пути `sendIgDirectExternal` (ответ из дашборда).
  Парсер `_b24ChatIdToResolveAlias` уже умеет этот префикс — менять его не
  пришлось.
- **Затронуто.** `src/bitrix24/bitrix24.service.ts` — `handleI2crmOutgoing`,
  `sendIgDirectExternal`.
- **Что НЕ повторять.** Проверять best-effort эмиссию по тому, что «код
  вызывается». Здесь он вызывался все полтора года. Проверять надо по базе:
  есть ли строки. И шире — **комментарий в коде не доказательство**: тот,
  что обещал `sent` для IG, был написан одновременно с вызовом, который его
  не отправлял.

## 2026-09-16 · Правки уехали в vendored checkout, который никуда не деплоится

- **Симптом.** Добавлял `/webhooks/internal/send-ig`. Написал код, проверил типы,
  собрался деплоить — и только тогда прочитал `CLAUDE.md` той папки: правил
  `pervyi-begovoy/bots/greenapi-b24/`, а это локальная копия для чтения,
  реальный репозиторий отдельный (`dv1-lab/greenapi-integration-bitrix24`).
- **Корень.** Обе копии файл в файл совпадают (проверил `md5` нетронутого
  `widget.controller.ts`), поиск по коду находит обе, и ничто в самом файле не
  подсказывает, что правишь не ту. Запрет написан в `CLAUDE.md` рядом — я открыл
  его уже после того, как закончил править.
- **Фикс.** Перенёс два файла в настоящий репозиторий, вернул копию к версии
  репозитория. Деплой — `cd /home/dv/greenapi-b24/source && git pull`, сборка из
  корня (`docker compose up -d adapter --build`).
- **Что НЕ повторять.** Начинать правку сервиса, не открыв его `CLAUDE.md`. Здесь
  он требует читать `PRODUCT_RULES.md` первым делом и `CHECKLIST_WIDGET.md` при
  любой правке исходящего пути — оба пункта я выполнил задним числом, и чек-лист
  оказался нужен: он прямо требует одинаковый `chat.id` у входящих и исходящих,
  а без этого B24 завёл бы второго chat-user (класс регрессии, повторявшийся
  5+ раз за полгода).

## 2026-09-14 · Коннектор не собрался бы на новом сервере

- **Симптом** (найден при сборке комплекта для стороннего разворачивания, в бою не
  проявлялся): `docker build` на чистом сервере падает на первой строке —
  `FROM source-adapter:latest` тянет образ, который существует только на server-spb.
  Комментарий «если donor отсутствует — Docker откатится на pnpm install» был
  неверен: Docker так не умеет, а `pnpm install` в файле не было вовсе.
- **Причина**: план B от 26.05 (npm и CDN Prisma временно недоступны) стал
  единственным путём сборки. Пока жив server-spb, это незаметно; при
  восстановлении на новом сервере коннектор не поднимается.
- **Фикс**: основной `Dockerfile` — сборка с нуля (`pnpm install --frozen-lockfile`,
  `prisma generate`, `build`), прежний вариант сохранён как `Dockerfile.donor`
  для быстрой сборки на сервере, где образ уже есть. Проверено сборкой на
  server-spb (npm и CDN Prisma доступны) и полным запуском с тестами на чистом сервере.
- **Не пересломать**: не возвращать `FROM <локальный образ>` в основной Dockerfile.

## 2026-09-11 · Исходящие Instagram уходили в Customer-360 без имени оператора

- **Симптом.** В ленте карточки клиента у всех исходящих Instagram автор пустой.
  Владелец: «какой оператор ответил — не отображается». Замер 11.09.2026:
  478 исходящих IG за пять дней, поле `operator` пустое у 478 из 478. У WhatsApp
  за те же дни имя есть у всех.
- **Причина.** Не регрессия, а пропуск с самого начала: `_emitIgMessageEvent`
  никогда не заполнял `operator`. У WhatsApp имя приезжает подсказкой в мост
  (`/internal/operator-hint`), и мост подставляет его сам; у Instagram события
  пишет сам коннектор, и заполнить поле было некому — хотя автор в этот момент
  под рукой, в `MESSAGES[0].message.user_id`, и рядом же используется для
  auto-transfer Direct-сессии.
- **Фикс** (`a2bdde2`). `_emitIgMessageEvent` принимает `portalDomain` и
  `operatorUserId`, резолвит имя тем же `getOperatorName` (кэш B24 `user.get`,
  TTL час), пишет в событие «ФИО · из Bitrix24» — как у WhatsApp.
- **Затронутые файлы.** `src/bitrix24/bitrix24.service.ts`: опции эмиттера,
  тело события, вызов в `handleI2crmOutgoing`.
- **Verify.** Типы проходят; коннектор пересобран и поднят, Nest стартовал.
  Полная проверка — на первом же исходящем Instagram менеджера утром.
- **Что пересломать рискованно.** Резолв имени добавляет обращение к порталу
  внутри фонового эмиттера. Он best-effort и глотает исключения, но если
  `user.get` начнёт отвечать медленно — события IG будут отставать. Кэш на час
  это гасит, но при массовой рассылке с новых операторов стоит смотреть.
- **Мораль.** Пустая на 100 % колонка — почти всегда «никто не пишет», а не
  «данных нет». Тот же признак месяцем раньше вскрыл невыгружаемые телефоны.

---

## 2026-08-08 · Тесты не запускались полтора месяца: прогон обрывался до установки зависимостей

- **Симптом:** каждый push в main давал письмо «Run failed: Tests». Прогон падал за 8–10 секунд. Красные уведомления шли Дмитрию с 12.07, поймано случайно — по письму на коммит `b517dde`.
- **Корень, два подряд:** (а) версия pnpm задавалась дважды — ключом `version: 10` в workflow и полем `packageManager: pnpm@10.33.2` в `package.json`; `pnpm/action-setup@v4` на конфликте отказывается ставить пакет («Multiple versions of pnpm specified») и валит job до `pnpm install`; (б) после этого прогон дошёл до тестов и упал на `bitrix24.service.spec.ts` — набор импортирует `@prisma/client`, а клиент генерируется из схемы и в репозитории не лежит; шага `prisma generate` в workflow не было.
- **Фикс:** `455e26e` (убран `version`, источник истины — `packageManager`) + следующий коммит (шаг `pnpm prisma generate` перед тестами).
- **Verify:** прогон `31257644736` — success за 30 с; 187 тестов, 9 наборов из 9. До фикса стабильно 1 failed / 8 passed.
- **Что НЕ повторять:** (а) **пин версии менеджера пакетов держать в одном месте** — при `packageManager` в `package.json` дублировать её в action нельзя, это не «на всякий случай», а гарантированное падение; (б) **красный CI, который никто не чинит, равен отсутствию CI**: полтора месяца тесты не запускались вовсе, при этом в репозиторий шли правки логики лидов и Customer-360; (в) генерируемые артефакты (Prisma-клиент) требуют явного шага в CI — локально они уже лежат в `node_modules` от прошлых запусков и дефект не виден.

---

## 2026-08-08 · Медиа в карточках B24 пережили переезд фасада на свой домен

- **Симптом:** предупреждающий — дефекта в проде не было. Фасад omnisocial переезжал с `facade-os.9wb.ru` на `api.omnisocial.ru`, а адрес старого домена зашит в коде коннектора двумя regex.
- **Корень:** B24-облако не тянет медиа с фасада (там IP-фильтр), поэтому коннектор скачивает файл сам и переотдаёт через `social.9wb.ru`. Условие входа в эту ветку — совпадение хоста ссылки со списком; в списке был только старый домен. Смени фасад публичную базу раньше — и в карточки B24 пошли бы ссылки, которые облако не откроет, то есть вложения без вложений.
- **Фикс (sha `b517dde`):** обе точки (`handleOutgoingFromCabinet` и ветка `attachments`) принимают оба домена — `facade-os.9wb.ru` и `api.omnisocial.ru`.
- **Verify:** правка задеплоена **до** смены `FACADE_PUBLIC_BASE_MAP` на фасаде; после переключения мост держит все четыре инстанса `authorized` через новый домен, медиа-роуты на обоих доменах отвечают одинаково.
- **Что НЕ повторять:** при переезде любого нашего хоста на новый домен — сначала грепнуть по коду потребителей на старое имя и выкатить расширение списка, и только потом менять адрес у источника. Старый домен не гасить: на нём ссылки в уже созданных карточках B24.
