...
#### Пометка `!` — ответить комментатору в Direct
...
Отличие от пути 6б: пометка `!` **не создаёт** отдельную Direct-сессию
открытой линии в B24 — исходящее остаётся в чате Comment-линии. Когда клиент
ответит в Direct, его входящее заведёт Direct-сессию штатно.

Написать в Direct можно и путём 6б (виджет), и из чата Direct-линии (18).
