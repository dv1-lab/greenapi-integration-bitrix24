...
**Фото/файлы.** Картинка передаётся i2crm полем `url` — публичной ссылкой
(adapter берёт `downloadLink` из B24-webhook: `/pub/im.file.php?…&SIGN=…`,
открывается без авторизации, i2crm скачивает сам). Поле `photo` НЕ годится —
это имя только для multipart-загрузки байтов файла; в JSON i2crm его молча
игнорирует и отклоняет запрос «нет текста/файла». На каждое фото — отдельный
вызов `/target/feedback` (по одному `url`), текст уходит вместе с первым.
Фото поддерживает только Direct; в ответ на комментарий уходит лишь текст.
