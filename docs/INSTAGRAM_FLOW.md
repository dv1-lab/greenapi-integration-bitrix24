...
(part `photo`, `Content-Transfer-Encoding: base64` — формат из i2crm API
Blueprint, тело собирается вручную в `_postI2crmFeedbackMultipart`). Adapter
скачивает файл из B24 по `downloadLink` (`/pub/im.file.php?…&SIGN=…` —
публичная ссылка) и шлёт байты с правильным именем (`name` из webhook, с
расширением) и `Content-Type` (`mime`). Способ «по ссылке» (поле `url`) не
годится: i2crm определяет тип файла по расширению в URL, а ссылка B24 — без
расширения (`im.file.php`) → «файл не поддерживается». На каждое фото —
отдельный вызов `/target/feedback`, текст уходит с первым. Фото — только
Direct; в ответ на комментарий уходит лишь текст.
