...
## Обновление 2026-06-22 — WhatsApp отложен до миграции на omnisocial

Перепроверка по живым данным (CH `bitrix1begovoy` + `customer360`): **WhatsApp
по-прежнему НЕ покрыт**, код не менялся. Причина уточнена: orphan-механизм
архитектурно сделан под каналы без телефона (TG/MAX/IG — опознание по `chat_id`):
- стэш заполняется только при `provider==="max"||"telegram"` (`chatIdForUf`);
- `_maybeLinkOrphanLead` распознаёт сироту по `UF_CRM_TG/MAX/IG_CHAT_ID` —
  у WhatsApp своего chat_id-UF на портале нет (опознаётся по телефону), и берёт
  из стэша по `chatId`, который у WA = телефон ≠ ключ TG/MAX.

Потенциал мал, но каждая метка = клиент с сайта: за 5 дней после фикса 16.06
потеряно 2 (лиды **363070**, **363104**; метки в CH `customer_events.summary`
`1781714395161696032`, `1781777737800530391`). Лид создаёт открытая линия →
`NF_YM` пусто (наш путь создания не отрабатывает) → `YA_CID` не пишется.

**Решение (Дмитрий, 2026-06-22):** НЕ чинить на Green-API-пути — WhatsApp скоро
мигрирует на движок omnisocial (другой путь приёма). Захват WA-метки сделать
**частью миграции** (стэш по телефону → дозапись в лид), одним заходом, без
двойной работы. Пункт зафиксирован в spec миграции
...
- spec `omnisocial/.../2026-06-22-boevye-channels-migration.md` — миграция, где включат WA-захват.
