...
- **Контакт найден, но не тот** (коллизия по phone между двумя клиентами)
  → linker привяжет orphan к первому найденному. Снимается через ручной
  re-link контакта в B24 + повторный POST в /internal/relink-orphan-lead.
  Низкая вероятность — клиент с тем же phone + тем же chat.id это редкость.
