...
10. **Я.Метрика ClientID пишем в `UF_CRM_YA_CID`** (+ `UF_CRM_YA_COUNTER_ID`,
    счётчик `45469563`) — боевое поле сквозной аналитики, его читает приложение
    B242YA для офлайн-конверсий, туда же пишет форма заказа сайта.
    `UF_CRM_NF_YM_CLIENT_ID` — **тупиковое** (в конверсиях не участвует), оставлено
    только как anti-block заглушка `"-"` для смены стадий — НЕ использовать как
