...
> {
	...
	/**
	 * Зеркало в B24 исходящего, отправленного НЕ из B24 а из наших omnisocial-
	 * поверхностей — веб-инбокса кабинета или TG-топика моста (omniSource ∈
	 * {cabinet, bridge}). Фасад шлёт синтетический outgoingMessageReceived с
	 * меткой omniSource. В отличие от handleOutgoingFromDevice: wa НЕ пропускаем
	 * (это наш omni-путь), userKey = wa_<phone> чтобы попасть в ТУ ЖЕ сессию что
	 * и входящие (иначе B24 заведёт второго chat-user). B24-исходящие сюда НЕ
	 * попадают (фасад их в adapter не фанит) — иначе был бы дубль.
	 */
	async handleOutgoingFromCabinet(webhook: any): Promise<void> {
		...
		const clientChatId = rawChatId.replace(/@c\.us$/, "");
		if (!clientChatId) return;
		// userKey должен совпадать с префиксом ВХОДЯЩИХ для этого канала:
		// wa → wa_<phone>, прочие (tg/max через surface) → sc_<chat>.
		...
		const origin = String(webhook?.omniSource || "cabinet");
		const portalDomain = inst.user?.portalDomain
			|| this.configService.get<string>("BITRIX_PORTAL_DOMAIN") || "1begovoy.bitrix24.ru";
		const displayName = String(senderData?.chatName || senderData?.senderName || clientChatId).trim() || clientChatId;
